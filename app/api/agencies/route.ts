...
export async function GET(request: Request) {
  ...
  if (name) {
    filteredAgencies = filteredAgencies.filter((agency) => agency.name.toLowerCase().includes(name.toLowerCase()))
  }
  ...
  if (specialization) {
    filteredAgencies = filteredAgencies.filter((agency) =>
      agency.specializations.some((s) => s.toLowerCase().includes(specialization.toLowerCase())),
    )
  }
