...
import { Calendar, Clock, MapPin, Plus, Users, X, AlertCircle, Check, ExternalLink, RefreshCw } from "lucide-react"
...
const initialEvents = [
  {
    ...
    type: "open-house",
    description: "Luxury beachfront property with 4 bedrooms and stunning ocean views.",
...
const eventTypes = {
  "open-house": { label: "Open House", color: "bg-blue-100 text-blue-700 border-blue-300" },
  "client-meeting": { label: "Client Meeting", color: "bg-green-100 text-green-700 border-green-300" },
  valuation: { label: "Property Valuation", color: "bg-purple-100 text-purple-700 border-purple-300" },
  internal: { label: "Internal Meeting", color: "bg-gray-100 text-gray-700 border-gray-300" },
  "follow-up": { label: "Follow-up", color: "bg-amber-100 text-amber-700 border-amber-300" },
...
const importedEvents = [
  {
    ...
    type: "internal",
    description: "Quarterly strategy planning session with the leadership team.",
...
export function CalendarView() {
  ...
  const [activeSources, setActiveSources] = useState(
    calendarSources.filter((source) => source.connected).map((source) => source.id),
  )
  ...
  const toggleCalendarSource = (sourceId) => {
  ...
  const syncCalendars = () => {
    ...
    alert("Syncing calendars... This would connect to external calendar APIs in production.")
  }
  ...
  const connectCalendar = (calendarId) => {
    ...
    alert(`Connecting to ${calendarId}... This would initiate OAuth in production.`)

    // Update the calendar source to connected
    const updatedSources = calendarSources.map((source) =>
      source.id === calendarId ? { ...source, connected: true, lastSynced: new Date().toISOString() } : source,
    )
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage your appointments, open houses, and schedule</p>
        </div>
        ...
        <Dialog open={isAddEventOpen} onOpenChange={setIsAddEventOpen}>
          ...
          <DialogContent className="sm:max-w-[525px]">
            <DialogHeader>
              ...
              <DialogDescription>Create a new event on your calendar. Click save when you're done.</DialogDescription>
            </DialogHeader>
          ...
          <CardHeader className="pb-3">
            <div className="flex justify-between items-center">
              <div>
                ...
                <CardDescription>Connect and manage your external calendars</CardDescription>
              </div>
              <div className="flex items-center gap-2">
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button variant="outline" size="sm" onClick={syncCalendars}>
                ...
                <Dialog>
                  ...
                  <DialogContent>
                    <DialogHeader>
                      ...
                      <DialogDescription>
                        Link your external calendars to view all your events in one place.
                      </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4 py-4">
                      {calendarSources
                        .filter((source) => !source.default && !source.connected)
                        .map((source) => (
                          <div key={source.id} className="flex items-center justify-between">
                            ...
                            <Button size="sm" onClick={() => connectCalendar(source.id)}>
                              Connect
                            </Button>
                          </div>
                        ))}
                      {calendarSources.filter((source) => !source.default && !source.connected).length === 0 && (
                        <div className="text-center py-4">
                          <Check className="h-8 w-8 text-green-500 mx-auto mb-2" />
                          <p className="font-medium">All available calendars are connected!</p>
                        </div>
                      )}
                    </div>
                    <DialogFooter>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {calendarSources.map((source) => (
                <div key={source.id} className="flex items-center justify-between space-x-2">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    {!source.connected && (
                      <Button variant="ghost" size="sm" className="h-8" onClick={() => connectCalendar(source.id)}>
                        Connect
                      </Button>
                    )}
                  </div>
                            ...
                            <PopoverContent className="w-80">
                              <div className="grid gap-4">
                                <div className="space-y-2">
                                  <h4 className="font-medium leading-none">Add Event on {format(day, "PPP")}</h4>
                                  <p className="text-sm text-muted-foreground">Create a new event on this date.</p>
                                </div>
                                <div className="grid gap-2">
                                  <Button
                                    onClick={() => {
                                      setNewEvent({
                                        ...
                                        endTime: format(day, "yyyy-MM-dd'T'10:00"),
                                      })
                          ...
                          <Dialog key={event.id}>
                            <DialogTrigger asChild>
                              ...
                              >
                                <div className="flex items-center w-full">
                                  {event.source && event.source !== "realtymate" && (
                                    <div
                            ...
                            <DialogContent>
                              ...
                              <div className="space-y-4 py-4">
                                ...
                                <div className="flex items-center gap-2">
                                  <div className={cn("px-2 py-1 rounded-full text-xs", eventTypes[event.type].color)}>
                                  ...
                                  <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                      ...
                                      <span className="text-sm">
                                        From{" "}
                                        {calendarSources.find((s) => s.id === event.source)?.name ||
                                          "External Calendar"}
                                      </span>
                                    </div>
                                    {event.readonly && (
                            ...
                            <DialogTrigger asChild>
                              ...
                              >
                                <div className="w-full">
                                  ...
                                  {event.location && (
                                    <div className="text-muted-foreground truncate mt-1">{event.location}</div>
                                  )}
                                </div>
                              </Button>
                            </DialogTrigger>
              ...
              <div className="space-y-4">
                {getEventsForDay(currentDate).length > 0 ? (
                  ...
                  <div className="text-center py-12">
                    ...
                    <p className="text-muted-foreground mb-4">You don't have any events scheduled for today.</p>
                    <Button
                      onClick={() => {
                        setNewEvent({
                          ...newEvent,
