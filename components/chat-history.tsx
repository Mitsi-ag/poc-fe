...
import { Search, MessageSquare, Calendar, Clock, MoreHorizontal, PlusCircle } from "lucide-react"
...
export function ChatHistory({ onChatSelect }: ChatHistoryProps) {
  ...
  const filteredChats = chatHistory.filter((chat) => {
    ...
    if (activeTab === "market") return matchesSearch && chat.tags.includes("Market Analysis")
    if (activeTab === "pitch") return matchesSearch && chat.tags.includes("Pitch")
    if (activeTab === "email") return matchesSearch && chat.tags.includes("Email")
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <Button variant="outline" className="gap-2" onClick={() => onChatSelect?.(`new-chat-${Date.now()}`)}>
        ...
        <TabsContent value={activeTab} className="mt-4">
          {filteredChats.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <Button className="mt-4 gap-2" onClick={() => onChatSelect?.(`new-chat-${Date.now()}`)}>
                <PlusCircle className="h-4 w-4" />
                <span>New Chat</span>
              </Button>
            </div>
