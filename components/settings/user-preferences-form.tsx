...
const popularSuburbs = {
  "New South Wales": ["Sydney", "Newcastle", "Wollongong", "Byron Bay", "Orange"],
  Victoria: ["Melbourne", "Geelong", "Ballarat", "Bendigo", "Mornington Peninsula"],
  Queensland: ["Brisbane", "Gold Coast", "Sunshine Coast", "Cairns", "Townsville"],
  "Western Australia": ["Perth", "Fremantle", "Margaret River", "Broome", "Albany"],
  "South Australia": ["Adelaide", "Barossa Valley", "Mount Gambier", "Port Lincoln", "Victor Harbor"],
  Tasmania: ["Hobart", "Launceston", "Devonport", "Burnie", "Strahan"],
  "Australian Capital Territory": ["Canberra", "Belconnen", "Woden", "Gungahlin", "Tuggeranong"],
  "Northern Territory": ["Darwin", "Alice Springs", "Katherine", "Nhulunbuy", "Tennant Creek"],
}

// Property types
const propertyTypes = ["Residential", "Commercial", "Industrial", "Land", "Rural", "Development Sites"]
...
export function UserPreferencesForm({ userData, updateUserData }: UserPreferencesFormProps) {
  const [selectedState, setSelectedState] = useState<string>(
    userData?.locations?.length > 0
      ? Object.keys(popularSuburbs).find((state) =>
          popularSuburbs[state as keyof typeof popularSuburbs].some((suburb) => userData.locations.includes(suburb)),
        ) || ""
      : "",
  )

  const [availableSuburbs, setAvailableSuburbs] = useState<string[]>(
    selectedState ? popularSuburbs[selectedState as keyof typeof popularSuburbs] : [],
  )
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="space-y-4">
          ...
          <FormField
            ...
            render={() => (
              <FormItem>
                <FormDescription>Select the goals you want to achieve with RealtyMate</FormDescription>
        ...
        <div className="space-y-4">
          ...
          <FormField
            ...
            render={() => (
              <FormItem>
                <FormDescription>Select which widgets you want to see on your dashboard</FormDescription>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-2">
                  {dashboardWidgets.map((widget) => (
                    <FormField
                      ...
                      render={({ field }) => {
                        return (
                          <FormItem key={widget} className="flex flex-row items-start space-x-3 space-y-0">
                            <FormControl>
                              <Checkbox
                                ...
                                onCheckedChange={(checked) => {
                                  return checked
                                    ...
                                    : field.onChange(field.value?.filter((value) => value !== widget))
                                }}
                              />
                            </FormControl>
                            <FormLabel className="font-normal">{widget}</FormLabel>
                          </FormItem>
                        )
                      }}
                    />
